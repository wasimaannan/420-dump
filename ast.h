...
public:
    ...
    string generate_index_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        if(this->get_type() == "int") {
            string temporary_idx = index->generate_code(outcode, symbol_to_temp, temp_count, label_count);
            string temporary_var = new_temp(temp_count);
            outcode << temporary_var << " = " << temporary_idx << " * 4" << endl; //t2=t*4
            return temporary_var;
        } else if(this->get_type() == "float") {
            string temporary_idx = index->generate_code(outcode, symbol_to_temp, temp_count, label_count);
            string temporary_var = new_temp(temp_count);
            outcode << temporary_var << " = " << temporary_idx << " * 8" << endl; 
            return temporary_var;
        }
        return "";
    }
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        if (index) {
            string temporary_idx = generate_index_code(outcode, symbol_to_temp, temp_count, label_count);
            string temporary_var = new_temp(temp_count); 
            outcode << temporary_var << " = " << name + "[" + temporary_idx + "]" << endl; //t3=a[t2]
            string arr_idx = this->name + "[" + temporary_idx + "]";
            symbol_to_temp[arr_idx] = temporary_var; 
            return temporary_var;
        } else {
            ...
            if (it != symbol_to_temp.end()) {
                ...
                string temporary_var = new_temp(temp_count);
                outcode << temporary_var << " = " << name << endl;
                symbol_to_temp[name] = temporary_var;
                return temporary_var;
            }
        }
        return "";
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        string temporary_var = new_temp(temp_count);
        outcode << temporary_var << " = " << value << endl; //t4=8
        return temporary_var;
    }
};
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        string first_val = left->generate_code(outcode, symbol_to_temp, temp_count, label_count);
        string second_val = right->generate_code(outcode, symbol_to_temp, temp_count, label_count);
        
        string temporary_var = new_temp(temp_count);
        outcode << temporary_var << " = " << first_val << " " << op << " " << second_val << endl;
        return temporary_var; //t4=t1+t2
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        string temporary_var = new_temp(temp_count);
        outcode << temporary_var << " = " << op << val_temp << ";" << endl; //t4=!a;
        return temporary_var;
    }
};
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        string lhs_var = lhs->get_name(); //a
        string rhs_val = rhs->generate_code(outcode, symbol_to_temp, temp_count, label_count); //t3
        
        if(lhs->has_index()) {
            string temporary_idx = lhs->generate_index_code(outcode, symbol_to_temp, temp_count, label_count);
            outcode << lhs_var << "[" << temporary_idx << "] = " << rhs_val << endl; //a[t4]=t3
            string full_lhs = lhs_var + "[" + temporary_idx + "]"; //a[t4]
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        // Should generate code for if-else statements
         if (else_block == nullptr) {
            ...
            outcode << "if " << cond_temp << " goto " << label_true << endl;
            temp_count++;
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        string label_loop_condition = "L" + to_string(label_count++);
        string label_loop_body = "L" + to_string(label_count++);
        string label_exit_loop = "L" + to_string(label_count++);

        outcode << label_loop_condition << ":" << endl;
        string cond_temp = condition->generate_code(outcode, symbol_to_temp, temp_count, label_count);

        outcode << "if " << cond_temp << " goto " << label_loop_body << endl;
        ...
        outcode << "goto " << label_loop_condition << endl;
...
public:
    ...
    string cond_temp = condition->generate_code(outcode, symbol_to_temp, temp_count, label_count);
    outcode << "if " << cond_temp << " goto " << label_for_body << endl;
...
private:
    string type;
    vector<pair<string, int>> vars; // Variable name and array size (0 for regular vars) //int a,b,c[10];
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        for( auto var : vars) {
            if (var.second > 0) {
                outcode <<"// Declaration: "<<type<<" "<< var.first <<"["<<var.second<<"]"<<endl; //c[10]
            } else {
...
class FuncDeclNode : public ASTNode { //int main();
private:
    string return_type;
    string name;
...
private:
    string func_name;
    vector<ExprNode*> arguments; //x=call foo,2
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        string temp_var = new_temp(temp_count);
        outcode << temp_var << " = call " << func_name << ", " << arguments.size() << endl; //t1=call foo,2
        return temp_var;
    }
};
